...
The diagrams describe the target layout. Currently the domain model and calculations, `TrackerController`, read-only dashboard, transactions, and assets screens, the canonical local `.xlsx` reader with its resource limits, user file selection, read-only portfolio grouping and reports, and the computed snapshot in `domain/snapshot.dart` exist. Portfolio editing, source mapping, and widget configuration are planned.

The repository carries `lib/` and `test/` only: there are no platform runner directories, so `flutter test` and `flutter analyze` run as they stand and running the app needs `flutter create .` to generate them first. That is also what registers the `file_selector` platform implementations. Implement the [product delivery order](product.md#delivery-order) incrementally; do not scaffold future components.
...
│   │   ├── workbook_picker.dart  # implemented: user file selection
...
The following is the recommended performance work. The streaming worksheet reader, the resource limits, and the release of consumed rows are implemented; progress, cancellation, isolates, and caching are not. It belongs to the existing storage and reporting work; it does not change the product delivery order or add workbook writing.

The opener no longer parses a whole worksheet or the shared-string table into a document tree: `xlsx_parts.dart` pulls those parts event by event and builds a tree for one `<row>` or `<si>` at a time, drops each decompressed part once read, and `openWorkbookFile` reads the container from a file stream rather than from the whole compressed file in memory. It still builds a cell grid for every worksheet before the canonical document is created, so peak memory remains larger than the compressed file; the reader now releases each row and each tab as it consumes them and yields row maps one at a time, so nothing beyond the document survives the load. Loading and initial calculations finish before `runApp`, and the controller retains the whole document afterward. The transactions list builds widgets lazily, but its backing data remains in memory.

Prioritize these changes:

1. Done as far as the reader goes: `xlsx_store.dart` yields one row map at a time and drops each source row and tab as its values are consumed. The decoder still builds each worksheet's grid whole before any of that runs, so peak memory is unchanged; streaming sheet rows from `xlsx_parts.dart` straight into the field readers is the remaining step, and it waits on a measurement.
...
3. Done: `WorkbookLimits` caps compressed input, per-part and total decompressed bytes, archive entries, shared strings, cell text length, and rows per sheet. Byte limits are checked against the ZIP's declaration and against the bytes actually produced, because a small archive may declare small parts and decompress without bound; a part whose declaration lies is still expanded once before it is refused. The entry limit is read from the container's own End of Central Directory records before the archive is decoded, and converted to entries through the 46-byte minimum of a directory record: the decoder allocates per record and then collapses repeated filenames, so any count taken from the decoded archive is both too late and too low. The probe follows the decoder rather than the specification — every candidate record in the last 64 KiB counts, scanning to the container's final four bytes because the decoder's scan reaches that far, and ZIP64 records replace the 32-bit fields whenever a locator precedes one, not only when those fields hold the sentinel. A container with no directory record in that range, or one that ends inside a record, is refused as unreadable: the decoder's file stream reads zeros past the end, so a record running off the file names a directory nothing here can bound. The directory is then read directly, ahead of the decode, so that an entry marked as a Unix symbolic link can be refused: the decoder expands such an entry's payload whole while building the archive, before any byte limit applies, and no part of a workbook is a link. A cell reference past the last column of a sheet is refused rather than padded into an unbounded row, and a part that stops mid-sheet is refused rather than read as the rows that arrived. Exceeding a limit rejects the load with `WorkbookFormatException` and never truncates records into an apparently valid tracker. If diagnostics are ever capped, state that further errors were omitted.
...
| --- | --- |
| Web | `pickWorkbook` selects through `file_selector` and feeds the bytes to `readWorkbook`, so the browser path no longer needs `dart:io` — untried so far, since the repository carries no web runner to build. That holds the whole compressed workbook in memory — the retention the file-backed reader exists to avoid — and the limits above are what bound it. Use a Web Worker for CPU work; Flutter `compute()` runs on the main thread on web. |
| Mobile | `pickWorkbook` covers selection; document-provider access and platform sandbox permissions still need handling, copying to app-local storage only when needed. Test memory pressure, cancellation, and lifecycle interruptions on representative phones. |
