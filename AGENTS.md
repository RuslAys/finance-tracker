...
Implemented: the Flutter client's canonical domain layer in `app/lib/domain/`, including read-only investment portfolios and their combined summary, a read-only Material 3 UI over it in `app/lib/features/`, a read-only local `.xlsx` reader in `app/lib/storage/`, and their tests. The app opens a workbook the user picks through `file_selector`, or one given by `--dart-define=tracker=<path>` at startup, and the synthetic sample otherwise. Do not add workbook writing, Google Sheets, import, ADK, MCP, or API implementation unless the user explicitly requests it.
