...
Documentation plus the implemented slices of the Flutter client: `app/lib/domain/` (canonical model, schema validation, exact decimals, finance engine, FX conversion, read-only portfolio grouping and reports, the scoped AI snapshot) and `app/lib/features/` (`TrackerController` plus read-only dashboard, transactions, and assets screens, with portfolio selection on Assets), with tests in `app/test/`. `app/pubspec.yaml` is a Flutter package using `flutter_lints`; run `flutter test` and `flutter analyze` from `app/`. `app/lib/storage/` reads a canonical `.xlsx` workbook into a `TrackerDocument` and nothing more: no writing, no Google Sheets, no import. `xlsx_parts.dart` reads the SpreadsheetML parts directly over `archive` and `xml` — the money rules need cell kind, formula presence, and the workbook date system, which spreadsheet packages hide. `main.dart` opens the path in `--dart-define=tracker`, or the synthetic `features/sample_tracker.dart` document when it is empty; `storage/workbook_picker.dart` opens a file the user picks, reading bytes on web and the path everywhere else. Adding workbook writing, editing, import, or companion code is an explicit user decision, not a prerequisite for other work.
...
Client-first, no product-operated backend. The Flutter app owns all finance rules, import validation, and analytics; a tracker's source of truth is either a local `.xlsx` workbook or one cloud spreadsheet (Google Sheets first; Microsoft Graph planned) — never both, and v1 has no automatic two-way sync. AI access is BYOK through an in-app `LlmProvider` abstraction (OpenAI-compatible, Anthropic, Gemini, local models such as Ollama), with keys in platform secure storage and config holding only `key_ref` references. The earlier local companion (ADK analytics + read-only MCP tools) is superseded; `docs/local-companion.md` is retained for reference and any LLM still operates only on a computed read-only snapshot, never persisting or writing tracker data.
