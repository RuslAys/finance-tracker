...
Documentation plus the implemented slices of the Flutter client: `app/lib/domain/` (canonical model, schema validation, exact decimals, finance engine, FX conversion, read-only portfolio grouping and reports) and `app/lib/features/` (`TrackerController` plus read-only dashboard, transactions, and assets screens, with portfolio selection on Assets), with tests in `app/test/`. `app/pubspec.yaml` is a Flutter package using `flutter_lints`; run `flutter test` and `flutter analyze` from `app/`. `app/lib/storage/` reads a canonical `.xlsx` workbook into a `TrackerDocument` and nothing more: no writing, no Google Sheets, no import. `xlsx_parts.dart` reads the SpreadsheetML parts directly over `archive` and `xml` — the money rules need cell kind, formula presence, and the workbook date system, which spreadsheet packages hide. `main.dart` opens the path in `--dart-define=tracker`, or the synthetic `features/sample_tracker.dart` document when it is empty. Adding workbook writing, editing, import, or companion code is an explicit user decision, not a prerequisite for other work.
